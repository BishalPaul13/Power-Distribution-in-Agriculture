    ...
    <div className="flex flex-col lg:flex-row min-h-[calc(100vh-64px)] bg-slate-50 font-sans">
      ...
      <aside className="lg:w-64 lg:sticky lg:top-16 lg:h-[calc(100vh-64px)] bg-slate-900 text-slate-300 hidden lg:flex flex-col flex-shrink-0 overflow-y-auto">
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button onClick={() => setActiveTab("dashboard")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === "dashboard" ? "bg-emerald-600/20 text-emerald-400" : "hover:bg-slate-800 text-slate-300"}`}>
            <LayoutDashboard size={18} />
            <span className="font-medium text-sm">Dashboard</span>
          </button>
          <button onClick={() => setActiveTab("farmers")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === "farmers" ? "bg-emerald-600/20 text-emerald-400" : "hover:bg-slate-800 text-slate-300"}`}>
            <Users size={18} />
      ...
      <main className="flex-1 p-4 sm:p-8">
        <div className="max-w-7xl mx-auto">
          ...
          <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              <h2 className="text-2xl font-bold text-slate-900">{activeTab === "dashboard" ? "Dashboard" : "Farmers"}</h2>
              <p className="text-slate-500 text-sm">{activeTab === "dashboard" ? "Manage power requests and system status." : "View all registered farmers."}</p>
            </div>
          </header>
                ...
                <>
                  ...
                  <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="p-5 border-b border-slate-100">
                      <p className="text-xs text-slate-500 mb-2">Total farmers: {farmers.length}</p>
                    </div>
                          ...
                          {farmers.length > 0 ? (
                            farmers.map((farmer) => (
                              <tr key={farmer._id} className="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                  <div className="text-sm text-slate-600">{farmer.email}</div>
                                </td>
                                <td className="px-6 py-4">
